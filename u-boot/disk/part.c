	       ...
	       block_dev_desc_t **dev_desc)
{
	...
	if (*ep) {
		if(show_partition_error) printf("** Bad device specification %s %s **\n",
		       ifname, dev_str);
		return -1;
	}

debug("get_device for %s %d\n",ifname, dev);

	*dev_desc = get_dev(ifname, dev);

if(*dev_desc)
{
debug("get_dev returns %s:%s\n",(*dev_desc)->vendor,(*dev_desc)->product);
}
else
{
debug("get_dev returns NULL\n");
}

	if (!(*dev_desc) || ((*dev_desc)->type == DEV_TYPE_UNKNOWN)) {
		if(show_partition_error) printf("** Bad device %s %s **\n", ifname, dev_str);
		return -1;
	}
			     ...
			     disk_partition_t *info, int allow_whole_dev)
{
	...
	if (!dev_part_str) {
		if(show_partition_error) printf("** No device specified **\n");
		goto cleanup;
	}
	...
	if (!part_str || !*part_str) {
		...
		if (*ep || (part == 0 && !allow_whole_dev)) {
			if(show_partition_error) printf("** Bad partition specification %s %s **\n",
			    ifname, dev_part_str);
			goto cleanup;
		}
	}
	...
	if (((*dev_desc)->part_type == PART_TYPE_UNKNOWN) ||
	    (part == 0)) {
		if (!(*dev_desc)->lba) {
			if(show_partition_error) printf("** Bad device size - %s %s **\n", ifname,
			       dev_str);
			goto cleanup;
		}
		...
		if ((part > 0) || (!allow_whole_dev)) {
			if(show_partition_error) printf("** No partition table - %s %s **\n", ifname,
			       dev_str);
			goto cleanup;
		}
	...
	if (part != PART_AUTO) {
		...
		if (ret) {
			if(show_partition_error) printf("** Invalid partition %d **\n", part);
			goto cleanup;
		}
	} else {
		/*
		...
		if (part) {
			...
			if(show_partition_error) printf("** No valid partitions found **\n");
			ret = -1;
			goto cleanup;
		}
	}
	if (strncmp((char *)info->type, BOOT_PART_TYPE, sizeof(info->type)) != 0) {
		if(show_partition_error) printf("** Invalid partition type \"%.32s\""
