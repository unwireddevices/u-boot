...
int usb_new_device(struct usb_device *dev)
{
	...
	/* Workaround for STALL during enumeration
	 * retry reading descriptor
	 * retry resetting port
	 */	
	port_reset_timeout = 10;
	err=-1;
	do{
		stall_timeout = 1;
		do{
			err = usb_get_descriptor(dev, USB_DT_DEVICE, 0, desc, 64);
		} while(err<0 && stall_timeout--);
		...
		if (err<0){
			//Reset port 
		  	if (parent) {
				...
				err2 = hub_port_reset(dev->parent, port, &portstatus);
				if (err2 < 0) {
	...
	} while (err<0 && port_reset_timeout--);
